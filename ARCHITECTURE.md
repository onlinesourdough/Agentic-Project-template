...
This document is the technical shape of the template. It is written for a coding agent or technical collaborator. The README stays founder-friendly.
...
This template is product-first and supports more than subscription products. Treat the stack as a set of capabilities to adopt deliberately.

Common shapes:

| Shape                      | Typical capabilities                                                             | Usually skip at first                                |
| -------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------- |
...
| Integration/API service    | Server routes, shared contracts, adapters, service tests, deployment automation  | React UI beyond admin/debug screens                  |
...
Use test-driven development where behavior matters: domain rules, services, adapters, webhooks, paid access, migrations, and bug fixes.

Scale the test layers to the product shape. A public landing page may only need contract tests, service tests, and a build check. A SaaS app should add route, adapter, webhook, auth, billing, and end-to-end coverage as those capabilities appear.
